...
const participantSchema = z.object({
  participantType: z.enum(["PARTICIPANT", "COACH", "JURY", "ORGANIZER", "GUEST"]).optional(),
...
function validateImportRow(data: Record<string, any>, rowNumber: number, importCategory?: string) {
  const errors: string[] = [];
  if (!data.fullName) errors.push(`Ligne ${rowNumber}: le nom complet est obligatoire.`);
  if (data.phone && data.phone.replace(/[^\d+]/g, "").length < 7)
    errors.push(`Ligne ${rowNumber}: téléphone invalide (${data.phone}).`);
  const isCoachJury =
    importCategory === "Coach" ||
    importCategory === "Jury" ||
    data.sourceCategory === "Coach" ||
    data.sourceCategory === "Jury";
  if (!isCoachJury && ["Hackathon", "CTF"].includes(data.sourceCategory)) {
    if (!data.competitionMode)
      errors.push(
        `Ligne ${rowNumber}: le mode solo/equipe est obligatoire pour ${data.sourceCategory}.`,
      );
    if (data.competitionMode === "equipe" && !data.teamName)
      errors.push(`Ligne ${rowNumber}: le nom d'équipe est obligatoire en mode équipe.`);
  }
  return errors;
}
...
participantsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const rows = rowsRaw
      ...
      .filter((p) => status === "ALL" || p.currentStatus === status);
...
participantsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    const schema = z
      .object({ participants: z.array(participantSchema).min(1).optional() })
      .and(participantSchema.partial());
    const body = parseBody(schema, req.body);
    const rows = body.participants?.length
      ? body.participants
      : [body as z.infer<typeof participantSchema>];
    const created = [];
...
participantsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const existing = await prisma.participant.findMany({
      select: { email: true, phone: true, fullName: true, groupName: true },
    });
    ...
    // Read sourceCategory from the form body (sent by the frontend)
    const importCategory =
      typeof req.body?.sourceCategory === "string" ? req.body.sourceCategory.trim() : undefined;
    const isCoachJury = importCategory === "Coach" || importCategory === "Jury";

    sheet.eachRow((row, index) => {
      if (index === 1) return;
      const source =
        cellText(row, headerMap, ["Source", "Catégorie", "sourceCategory"]) ||
        cellText(row, headerMap, ["type"]) ||
        "";
      const isCtf =
        source.toLowerCase().includes("ctf") || cellText(row, headerMap, ["niveau", "categories"]);

      // Extract role value from excel
      const roleLabelRaw = nullableString(cellText(row, headerMap, ["Rôle", "role"]));
      const roleLower = (roleLabelRaw || "").trim().toLowerCase();
      const isRowCoach = roleLower === "coach";
      const isRowJury = roleLower === "jury";

      // Determine participantType based on import category or role column
      let participantType = "PARTICIPANT";
      let sourceCategory: string;
      if (isRowCoach) {
        participantType = "COACH";
        sourceCategory = "Coach";
      } else if (isRowJury) {
        participantType = "JURY";
        sourceCategory = "Jury";
      } else if (isCoachJury) {
        participantType = importCategory === "Coach" ? "COACH" : "JURY";
        sourceCategory = importCategory!;
      } else {
        sourceCategory = isCtf
          ? "CTF"
          : source.includes("Hackathon")
            ? "Hackathon"
            : source || "Hackathon";
      }

      // Determine roleLabel — for Coach/Jury, default to the category name if not set in the file
      const roleLabel =
        roleLabelRaw ||
        (sourceCategory === "Coach" || sourceCategory === "Jury" ? sourceCategory : null);
      ...
      const data: Record<string, any> = {
        ...
        competitionMode:
          nullableString(cellText(row, headerMap, ["Mode", "type"]))
            ?.replace("team", "equipe")
            .toLowerCase() || null,
        teamName: nullableString(cellText(row, headerMap, ["Équipe", "Equipe", "equipe"])),
        memberCount:
          Number(cellText(row, headerMap, ["Nombre membres", "memberCount"]) || 1) || null,
        competitionLevel: nullableString(
          cellText(row, headerMap, ["Thématique / Niveau", "thematique", "niveau"]),
        ),
        competitionCategories: nullableString(
          cellText(row, headerMap, ["Catégories / Spécialités", "categories", "Catégories"]),
        ),
        roleLabel,
        ...
        expectedPresence: (
          nullableString(cellText(row, headerMap, ["Présence prévue", "expectedPresence"])) ??
          "BOTH_DAYS"
        ).toUpperCase(),
        notes: nullableString(
          cellText(row, headerMap, ["Observation", "notes", "Reçu le", "recu_le"]),
        ),
      };

      // For Coach/Jury, nullify irrelevant competition fields
      if (sourceCategory === "Coach" || sourceCategory === "Jury") {
        data.competitionMode = null;
        data.memberCount = null;
      }

      const errors = validateImportRow(data, index, importCategory);
      const isRowCoachJury = sourceCategory === "Coach" || sourceCategory === "Jury";
      if (!isRowCoachJury && data.competitionMode === "equipe" && data.teamName) {
      ...
      if (
        existing.some(
          (p) =>
            ...
            (p.fullName === data.fullName && p.groupName === data.groupName),
        )
      ) {
...
participantsRouter.post(
  ...
  asyncHandler<AuthedRequest>(async (req, res) => {
    const schema = z.object({
      rows: z.array(participantSchema),
      sourceCategory: z.string().default("Import"),
    ...
    await prisma.$transaction(async (tx) => {
      for (const row of input.rows) {
        ...
        await tx.participant.create({
          data: {
            ...
            badgeCode: generateBadgeCode(nextNumber++),
            qrToken: generateQrToken(),
            sourceCategory: data.sourceCategory ?? input.sourceCategory,
          },
        });
        importedRows += 1;
      }
      await tx.importBatch.create({
        data: {
          ...
          importedRows,
          skippedRows: input.rows.length - importedRows,
          errorRows: 0,
        },
      });
    });
...
participantsRouter.post(
  "/:id/photo",
  requireRole("ADMIN", "SUPERVISOR", "SCAN_AGENT"),
  upload.single("photo"),
  asyncHandler(async (req, res) => {
    ...
    if (!meta?.format || !["jpeg", "png", "webp"].includes(meta.format))
      throw httpError(400, "Format photo invalide");
    ...
    const participant = await prisma.participant.update({
      where: { id: req.params.id },
      data: { photoPath: `/uploads/photos/${fileName}` },
    });
    res.json(serializeParticipant(participant));
  }),
);
