...
function ImportPage() {
  ...
  const pageRows = useMemo(
    () => previewRows.slice((previewPage - 1) * 5, previewPage * 5),
  ...
  async function previewFile() {
    ...
    setBusy(true);
    try {
      const result = await previewParticipantImport(file, category);
      setPreview(result);
      setPreviewPage(1);
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Plusieurs fichiers successifs sont supportés. Aucun écrasement sans confirmation.
          </p>
        </div>
      ...
      <Card className="p-5 space-y-4">
        <div className="grid sm:grid-cols-[1fr_2fr] gap-4">
          <div className="space-y-1.5">
            ...
            {(category === "Coach" || category === "Jury") && (
              <p className="text-xs text-muted-foreground mt-1.5 leading-relaxed">
                <span className="font-semibold">Format attendu :</span> Source, Référence, Mode,
                Équipe, Nombre membres, Thématique / Niveau, Catégories / Spécialités, Chef /
                Capitaine, Rôle, Nom complet, Email, Téléphone, Genre, Ville, Statut, Organisation /
                Établissement, Formation, Job dating, Reçu le, Observation.
                <br />
                Les champs Mode, Équipe et Nombre membres ne sont pas obligatoires pour les{" "}
                {category === "Coach" ? "coachs" : "jurys"}.
              </p>
            )}
          </div>
          <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="text-sm">
                  {file ? (
                    ...
                    "Cliquer pour sélectionner un fichier .xlsx, .xls ou .csv"
                  )}
                </div>
              </div>
            </label>
      ...
      {preview && (
        <Card className="overflow-hidden">
          <div className="px-5 py-3 border-b text-sm font-medium">
            Prévisualisation ({preview.rows.length} lignes, {preview.duplicates.length} doublon(s)
            possible(s))
          </div>
          <div className="divide-y">
            {pageRows.map((row, i) => {
              const data = row.data ?? {};
              const isCoachJury = category === "Coach" || category === "Jury";
              return (
                <div
                  key={`${previewPage}-${i}`}
                  className="px-5 py-3 grid grid-cols-[auto_minmax(0,2fr)] sm:grid-cols-[auto_2fr_2fr_1.5fr_1.5fr] gap-3 items-center text-sm"
                >
                  {row.ok ? (
                  ...
                  <div className="min-w-0">
                    <div className="truncate">
                      {String(data.fullName || "") || (
                        <span className="text-destructive">Ligne invalide</span>
                      )}
                    </div>
                    {row.error && (
                      <div className="text-xs text-destructive whitespace-normal">{row.error}</div>
                    )}
                  </div>
                  ...
                  <div className="hidden sm:block truncate">
                    {isCoachJury ? String(data.roleLabel || "—") : String(data.groupName || "—")}
                  </div>
                  <div className="hidden sm:block truncate">
                    {isCoachJury ? String(data.organization || "—") : String(data.teamName || "—")}
                  </div>
                </div>
              );
            })}
