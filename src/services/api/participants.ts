...
export async function listParticipants(
  q: ParticipantsQuery = {},
): Promise<PagedResult<Participant>> {
...
export async function quickAdd(
  input: QuickAddInput | { participants: QuickAddInput[] },
): Promise<Participant | { items: Participant[] }> {
  return http<Participant | { items: Participant[] }>("/participants/quick-add", {
    method: "POST",
...
export async function previewParticipantImport(
  ...
  if (sourceCategory) form.append("sourceCategory", sourceCategory);
...
export async function uploadPublicParticipantPhoto(
  qrToken: string,
  file: File,
): Promise<Participant> {
  const form = new FormData();
...
export async function sendParticipantBadgeEmail(
  id: string,
  options: { force?: boolean } = {},
): Promise<BadgeEmailResult> {
  const suffix = options.force ? "?force=true" : "";
  return http<BadgeEmailResult>(`/badges/${id}/email${suffix}`, { method: "POST" });
}

export async function sendGroupBadgeEmails(
  groupName: string,
  options: { force?: boolean } = {},
): Promise<{ sent: number; failed: number; results: BadgeEmailResult[] }> {
  const suffix = options.force ? "?force=true" : "";
